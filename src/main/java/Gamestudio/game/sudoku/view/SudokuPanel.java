package Gamestudio.game.sudoku.view;
...
import Gamestudio.game.sudoku.controller.SudokuController;
import Gamestudio.game.sudoku.model.Game;
import Gamestudio.game.sudoku.model.UpdateAction;
...
public class SudokuPanel extends JPanel implements Observer {
    ...
    private Gamestudio.Server.controller.SudokuController sudokuController;
    ...
    private void setGameCheck(Game game) {
    	sudokuController = new Gamestudio.Server.controller.SudokuController();
        for (int y = 0; y < 9; y++) {
            for (int x = 0; x < 9; x++) {
                fields[y][x].setBackground(Color.WHITE);
